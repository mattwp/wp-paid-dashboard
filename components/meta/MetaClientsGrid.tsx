...
import MetaClientCard from './MetaClientCard'
import type { MetaClientConfig, MetaClientSummary } from '@/lib/types'

interface Props {
  clients: MetaClientConfig[]
  summaries: Record<string, MetaClientSummary | null>
}
...
export default function MetaClientsGrid({ clients, summaries }: Props) {
  const [search, setSearch] = useState('')
  const [filter, setFilter] = useState<Filter>('all')
  const [leadFilter, setLeadFilter] = useState<string>('all')

  const leads = Array.from(new Set(clients.flatMap(c => c.leads))).sort()

  const filtered = clients.filter(c => {
    const matchesSearch = !search || c.name.toLowerCase().includes(search.toLowerCase())
    const matchesFilter = filter === 'all' || c.type === filter
    const matchesLead = leadFilter === 'all' || c.leads.includes(leadFilter)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map(c => (
          <MetaClientCard key={c.id} id={c.id} name={c.name} type={c.type} leads={c.leads} summary={summaries[c.id] ?? null} />
        ))}
      </div>
    </div>
  )
