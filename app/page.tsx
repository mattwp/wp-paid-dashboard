...
import { computeAlerts, computeOpportunities } from '@/lib/signals'
import type { ClientSummary, CampaignRow } from '@/lib/types'
...
export default function Home() {
  ...
  for (const c of clients) {
    ...
    campaignsByClient[c.id] = getClientCampaigns(c.id)
  }

  const alerts = computeAlerts(clients, summaries)
  const opportunities = computeOpportunities(clients, summaries, campaignsByClient)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px' }}>
        <div>
          ...
          <AlertsDrawer alerts={alerts} opportunities={opportunities} />
        </div>
